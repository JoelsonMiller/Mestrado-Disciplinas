...
%Descomentar para carregar os pesos salvos
 %w = load('pesos_treinados/regulation_exact_ator_nn.mat');
 %ganhos = w.Wc;

q_func = qFunctionClass(z_vector_size, nn_size, ganhos); %Iniciando a Classe do função Q

%Matrizes de função de custo 
Qc = diag([750*ones(1, state_size/2) 0.001*ones(1, state_size/2)]); %ponderação dos erros
...
%hiperparametros
gamma = 0.4;%gammas(g); %0.4 %fator de aprendizado
...
%
learning_cycle = 2;%learn_cycles(lc); %segundos
